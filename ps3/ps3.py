...
def get_word_score(word, n):
    ...
    for char in new_word.lower():
        sum_of_points += SCRABBLE_LETTER_VALUES[char]
    
    return sum_of_points * max(7 * len(word) - 3 * (n - len(word)), 1)
...
def is_valid_word(word, hand, word_list):
    ...
    letter_freq = get_frequency_dict(lower_word)

    for char in lower_word:
        if char not in hand or letter_freq[char] > hand[char]:
            return False
    
    for char in VOWELS:
        if lower_word.replace("*", char) in word_list:
            return True
    else:
        return False
...
def play_game(word_list):
    ...
    * Returns the total score for the series of hands
    ...
    for i in range(total_hands):
        ...
        total_score_for_entire_game += hand_score

        if i == total_hands - 1:
            print(f"Total score over all hands: {total_score_for_entire_game}")    
